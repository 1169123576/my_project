...
public class FileIoAutoTest {
	...
	public String test() {
		String data=FileIoAutoTest.uploadVideo();
		return data;
	}
	public static String uploadVideo() {
		...
		String strLogin = HttpClientMainClass.sendPost(globeUrl + "/v1/auth/login",
				"phone=" + GetPropertiesValue.getUserName() + "&password=" + GetPropertiesValue.getPassWord()
						+ "&version=2.0.0");
		...
		try {
			...
			// 调用上传视频入库接口，视频、图片入库
			uploadBody = HttpClientMainClass.sendJsonBodyPost(globeUrl + "/v1/video/uploadVideo", jsonImage,
					jsonVideo, title, uid, upLoadVideoApiPath);
